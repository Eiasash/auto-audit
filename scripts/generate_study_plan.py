...
def schedule(topics: list[dict], hours_per_week: float, weeks: int) -> list[list[dict]]:
    ...
    for t in sorted_topics:
        ...
        for i in range(weeks):
            # + 1e-9 float tolerance: a topic that mathematically fits exactly
            # (sum == weekly_budget + 0.5) can land ~1e-15 above it in IEEE-754
            # and get wrongly bumped to a later week. The JS port
            # (study_plan/algorithm.js) carries the same epsilon; without it the
            # two implementations diverge on boundary data (e.g. the Mishpacha
            # slice after the 2026-06 syllabus resync — JS week 5 vs Python
            # week 15). Pinned by each PWA's studyPlanAlgorithm.test.js.
            if used[i] + t["hours"] <= weekly_budget + 0.5 + 1e-9:
...
def render_md(app_key, app_meta, start, exam, hpw, weeks_arr, used, topics, ramp_weeks):
    ...
    L.append(
        f"- **Hours per week**: {hpw} (≈ {round(hpw*0.7,1)}h topics, {round(hpw*0.2,1)}h Q-bank, {round(hpw*0.1,1)}h misc)"
    )
    L.append(
        f"- **Daily Q-bank target**: {daily_q} questions on [{app_meta['deploy_url']}]({app_meta['deploy_url']})"
    )
    L.append("")
    L.append(
        f"## Topic distribution (empirical, n={sum(t['n_questions'] for t in topics):,} past-exam questions analyzed)"
    )
    L.append("")
    ...
    for i, week in enumerate(weeks_arr):
        ...
        for t in week:
            ...
            L.append(
                f"### {t['en']}{he} — {t['hours']}h ({t['frequency_pct']}% of past exams)"
            )
            if t.get("keywords"):
    ...
    if ramp_weeks > 0:
        for j in range(ramp_weeks):
            ...
            wk_end = wk_start + timedelta(days=6)
            L.append(
                f"## Ramp Week {j+1} — {wk_start.isoformat()} → {wk_end.isoformat()}"
            )
            L.append("")
            if j == 0:
                L.append(
                    f"- **Mock exam #1** — full timed at [{app_meta['deploy_url']}]({app_meta['deploy_url']})"
                )
                L.append("- Review every miss, mark for spaced repetition")
                L.append(
                    f"- Hot review: weakest 5 topics from mock #1 (typically the top-frequency ones you scored < 70%)"
                )
            elif j == 1:
                ...
                L.append(
                    "- **Mock exam #3** — full timed simulation under exam conditions"
                )
    ...
    L.append(
        "- Frequency-weighted hours protect against over-investing time in low-yield topics."
    )
    ...
    L.append("")
    L.append(
        f"_Generated by `scripts/generate_study_plan.py` from `syllabus_data.json` (refresh by re-running `scripts/refresh_syllabus_data.py`)._"
    )
    return "\n".join(L)
...
def main():
    ...
    if not os.path.exists(DATA_FILE):
        raise SystemExit(
            f"missing {DATA_FILE} — run scripts/refresh_syllabus_data.py first"
        )
    ...
    if key not in data:
        raise SystemExit(
            f"app {args.app} not in syllabus_data.json (have: {list(data.keys())})"
        )
    ...
    if total_weeks < args.ramp_weeks + 4:
        raise SystemExit(
            f"only {total_weeks} weeks until exam — need at least {args.ramp_weeks + 4}"
        )
    ...
    print(
        f"  total weeks: {total_weeks}, topic weeks: {topic_weeks}, ramp weeks: {args.ramp_weeks}"
    )
    print(
        f"  topics: {len(topics)}, hours/topic range: {min(t['hours'] for t in topics)}–{max(t['hours'] for t in topics)}h"
    )
